...
// NOTE AssistantMessage llm返回后信息的表示形式
export interface AssistantMessage {
	role: "assistant";
	content: (TextContent | ThinkingContent | ToolCall)[]; // 一般就是信息体
	api: Api;
	provider: ProviderId;
	model: string;
 ...
 *   and errorMessage.
 * // NOTE AssistantMessageEvent
 * 厂商服务器 --SSE(各家格式不同)--> api/*.ts 解析翻译 --> 统一事件流 --> UI 渲染
 * 不是直接传这些事件—— AssistantMessageEvent  是 pi-ai 在客户端翻译后的产物
 * 
 * 所有适配器的职责是把厂商流翻译成它（下面的事件）
 */
